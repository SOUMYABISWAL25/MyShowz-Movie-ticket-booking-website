...
var getObject = composeServiceApi(s3TransferHandler, getObjectSerializer, getObjectDeserializer, { ...defaultConfig2, responseType: "blob" });
var getPresignedGetObjectUrl = async (config2, input) => {
  const endpoint = defaultConfig2.endpointResolver(config2, input);
  ...
  if (config2.userAgentValue) {
    url.searchParams.append(config2.userAgentHeader ?? USER_AGENT_HEADER2, config2.userAgentValue);
  }
  ...
  return presignUrl({ method, url, body: void 0 }, {
    signingService: defaultConfig2.service,
    signingRegion: config2.region,
    ...defaultConfig2,
    ...config2
  });
};
...
var deleteObject = composeServiceApi(s3TransferHandler, deleteObjectSerializer, deleteObjectDeserializer, { ...defaultConfig2, responseType: "text" });
...
var remove = async (amplify, input) => {
  ...
  if (inputType === STORAGE_INPUT_KEY) {
    ...
    logger14.debug(`remove "${objectKey}" from "${finalKey}".`);
  } else {
    finalKey = objectKey;
    logger14.debug(`removing object in path "${finalKey}"`);
  }
  await deleteObject({
    ...s3Config,
...
var getProperties = async (amplify, input, action) => {
  ...
  logger14.debug(`get properties of ${objectKey} from ${finalKey}`);
...
var isCancelError = (error) => !!error && error instanceof CanceledError2;
